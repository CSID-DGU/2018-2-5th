...
function getCard() {
	var ownerAddress = document.getElementById('index').value;
  var cardNum = 0;

	blood.getCard(ownerAddress, 0, function(err, result) {
    if(!err) {
  	   alert(result);
    }
	});
...
<%
					...
					try {
						...
						String url = "jdbc:mysql://localhost:3306/ossp_blood?characterEncoding=utf8"; 
						conn = DriverManager.getConnection(url,"root","password!"); 
						stmt = conn.createStatement(); 
						sql = "select * from donationReserved"; 
	...
	<section id="accept" class="white-bg"style="margin-top:50px;">
          <div class="container-fluid" style="padding:80px; text-align:center;">
            <div class="row">
                ...
                <h2 class="black">헌혈자 목록</h2></div>

              <div class="table-responsive"style="Width:100%">
                <table class="mdl-data-table mdl-js-data-table mdl-shadow--2dp" style="display:inline-block; margin:40px;">
            <thead>
              <tr>
                <th>이름</th>
                <th>혈액형</th>
                <th>헌혈 종류</th>
                <th class="mdl-data-table__cell--non-numeric">헌혈증 지갑주소</th>
                <th> 등록 </th>
              </tr>
            </thead>
            <tbody>
              ...
              <tr>
                ...
                <td><%=rs.getString("donationType") %></td>
                <td><%=rs.getString("coinPocket")%> </td>
                <td><button type="submit" class="btn btn-primary btn-lg" onclick="creatCard('<%=rs.getString("donationType") %>', '<%=rs.getString("coinPocket")%>')">승인</button></td>
              </tr>
                <% } %>
            </tbody>
          </table>
